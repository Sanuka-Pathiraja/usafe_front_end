...
class _SafeRouteNavigationScreenState extends State<SafeRouteNavigationScreen>
  ...
  final GlobalKey _guideFabsKey = GlobalKey();
  final GlobalKey _guideLegendKey = GlobalKey();
  ...
  Future<void> _checkOnboarding() async {
    final should = await OnboardingController.shouldShowSafeRouteTour();
    if (should && mounted) {
      setState(() => _showOnboarding = true);
      await OnboardingController.markSafeRouteTourSeen();
    }
  }
        ...
        children: [
          ...
          Positioned(
            ...
            child: AnimatedOpacity(
              ...
              child: SizedBox(key: _guideLegendKey, child: _buildMapLegend()),
            ),
          ),
          ...
          // ── First-time cinematic guide overlay ────────────────────────
          if (_showOnboarding)
            _RouteOnboardingOverlay(
              onDone: _dismissOnboarding,
              stepKeys: [_guideSearchKey, _guideFabsKey, _guideLegendKey],
            ),
        ],
      ),
    );
...
// ─────────────────────────────────────────────────────────────────────────────
// Cinematic first-time guide overlay — Safe Route Navigation
// ─────────────────────────────────────────────────────────────────────────────

class _RouteTourStep {
  final IconData icon;
  final Color color;
  final String label;
  final String title;
  final String body;
  const _RouteTourStep({
    required this.icon,
    required this.color,
    required this.label,
    required this.title,
...
const _kRouteSteps = [
  _RouteTourStep(
    ...
    body:
        'Type where you want to go and select from suggestions.\nUSafe checks the route for danger zones automatically.',
  ),
  _RouteTourStep(
    ...
    body:
        'Pin a destination by tapping the map, jump to your location, or clear the current route — all with one tap.',
  ),
  _RouteTourStep(
    icon: Icons.legend_toggle_rounded,
    color: Color(0xFF10B981),
    label: 'Danger zone legend',
    title: 'Live danger zone legend',
    body:
        'See how many active danger zones are nearby and whether a safe alternative route was found for you.',
  ),
];

class _RouteSpotlightPainter extends CustomPainter {
  final Rect? highlight;
  final Color glowColor;
  final double glowT;

  const _RouteSpotlightPainter({
  ...
  @override
  bool shouldRepaint(_RouteSpotlightPainter old) =>
      old.highlight != highlight ||
      old.glowColor != glowColor ||
      old.glowT != glowT;
}

class _RouteOnboardingOverlay extends StatefulWidget {
  final VoidCallback onDone;
  final List<GlobalKey> stepKeys;

  const _RouteOnboardingOverlay({
    required this.onDone,
    required this.stepKeys,
  });

  @override
  State<_RouteOnboardingOverlay> createState() =>
      _RouteOnboardingOverlayState();
}

class _RouteOnboardingOverlayState extends State<_RouteOnboardingOverlay>
    with TickerProviderStateMixin {
  int _step = 0;
  Rect? _highlightRect;
  ...
  Future<void> _advance() async {
    if (_step < _kRouteSteps.length - 1) {
      setState(() {
        _step++;
        _highlightRect = null;
      });
  ...
  @override
  Widget build(BuildContext context) {
    final step = _kRouteSteps[_step];
    final color = step.color;
    final isLast = _step == _kRouteSteps.length - 1;
            ...
            child: GestureDetector(
              ...
              child: AnimatedBuilder(
                ...
                builder: (_, __) => CustomPaint(
                  painter: _RouteSpotlightPainter(
            ...
            child: Center(
              child: AnimatedSwitcher(
                ...
                child: Container(
                  ...
                  child: Text(
                    '${_step + 1} of ${_kRouteSteps.length}',
                    style: TextStyle(
                      color: color,
                      fontSize: 12,
                      fontWeight: FontWeight.w700,
