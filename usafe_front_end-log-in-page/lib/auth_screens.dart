...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _handleLogin() async {
    // FIX IS HERE: Added 'await'
    if (await MockDatabase.validateLogin(_emailController.text.trim(), _passwordController.text.trim())) {
      if (!mounted) return;
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const HomeScreen()));
    } else {
      if (!mounted) return;
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: ElevatedButton(
                      ...
                      child: const Text("LOGIN", style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                    ),
                  ),
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 20),
                  _buildModernInput(passCtrl, "Password", Icons.lock_outline, isPassword: true),
                  const SizedBox(height: 40),
                  Container(
                    width: double.infinity,
                    height: 55,
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () async {
                        if (nameCtrl.text.isNotEmpty && emailCtrl.text.isNotEmpty && passCtrl.text.isNotEmpty) {
                          await MockDatabase.registerUser(nameCtrl.text, emailCtrl.text, passCtrl.text);
